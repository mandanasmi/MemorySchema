...
class BDeScore(BaseScore):
    ...
    def get_local_scores(self, target, indices, indices_after=None):
        ...
        return (local_score_before, local_score_after) # delta score for BDe without the priors 
...
class TemporalBDeScore(BaseScore):
    def __init__(self, data, prior, equivalent_sample_size=1.):
        super().__init__(data, prior)
        self.equivalent_sample_size = equivalent_sample_size
        
        # Assumes data preprocessing to label columns by time t, t+1, etc.
        self.state_names = {column: sorted(self.data[column].cat.categories.tolist())
                            for column in self.data.columns}
        
    def get_local_scores(self, target, indices, indices_after=None):
        # Calculate state counts for nodes across different time steps
        state_counts_before, state_counts_after = self.state_counts(
            target, indices, indices_after=indices_after)
        
        local_score_after = self.local_score(*state_counts_after)
        local_score_before = self.local_score(*state_counts_before) if state_counts_before else None
    ...
    def state_counts(self, target, indices, indices_after=None):
        ...
        # Filter rows for non-interventional cases
        data = self.data.loc[self.data['interventional'] != target]
        state_count_data = (data.groupby([variable] + parents).size().unstack(parents))
        
        # Data processing to handle MultiIndex and fill missing states with zeros
        if isinstance(state_count_data, pd.DataFrame):
            if not isinstance(state_count_data.columns, pd.MultiIndex):
                state_count_data.columns = pd.MultiIndex.from_arrays([state_count_data.columns])
            parent_states = [self.state_names[parent] for parent in parents]
            columns_index = pd.MultiIndex.from_product(parent_states, names=parents)
            counts_after = state_count_data.reindex(index=self.state_names[variable], columns=columns_index).fillna(0)
        else:
            counts_after = state_count_data.to_frame()

        state_counts_after = StateCounts(key=(target, tuple(all_indices)), counts=counts_after)
        
        # Prepare for returning state counts before the intervention or temporal shift
        if indices_after:
            subset_parents = [self.column_names[index] for index in indices]
            data = state_counts_after.counts.groupby(axis=1, level=subset_parents).sum() if subset_parents else state_counts_after.counts.sum(axis=1).to_frame()
    ...
    def local_score(self, key, counts):
        # Similar scoring function but may adjust alpha and beta considering temporal dependencies
        counts = np.asarray(counts)
        num_parents_states = counts.shape[1]
        alpha = self.equivalent_sample_size / num_parents_states
        beta = self.equivalent_sample_size / counts.size
        ...
        log_gamma_conds = gammaln(np.sum(counts, axis=0) + alpha)
        
        local_score = np.sum(log_gamma_counts) - np.sum(log_gamma_conds) + num_parents_states * math.lgamma(alpha) - counts.size * math.lgamma(beta)
        
        return LocalScore(key=key, 
                        score=local_score,
                        prior=self.prior(num_parents))